...
public abstract class Traversal {
    ...
    public void traverse(Collection<Integer> V0) {
        ...
        while (!_fringe.isEmpty()) {
            int theone = _fringe.remove();
            if (!marked(theone)) {
                mark(theone);
                _fringe.add(theone);
                if (visit(theone)) {
                    processSuccessors(theone);
                } else {
                    return;
                }
            } else {
                if (!_postvisited.contains(theone)) {
                    if (_visited.contains(theone) && postVisit(theone)) {
                        _postvisited.add(theone);
                    }
                }
            }
        }
    ...
    /** Store the vertices visited. */
    private ArrayList<Integer> _visited;
    /** Store the vertices postvisited. */
