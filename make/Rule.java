...
class Rule {
    ...
    private void checkFinishedDependencies() {
        int vert = getVertex();
        for (Integer dum : _depends.successors(vert)) {
            if (_depends.getLabel(dum).isUnfinished()) {
                error("Error: Not all dependencies are in fact built before"
                       + " it's time to rebuild a node.");
            }
        }
    }
    ...
    private boolean outOfDate() {
        ...
        for (Integer dum : _depends.successors(_vertex)) {
            if (_depends.getLabel(dum).getTime() == null) {
                return true;
            }
            if (_depends.getLabel(dum).getTime() > getTime()) {
                return true;
            }
        }
        return false;
    ...
    void rebuild() {
        ...
        if (outOfDate()) {
            if (_commands.isEmpty()) {
                ...
                for (int i = 0; i < _commands.size(); i += 1) {
