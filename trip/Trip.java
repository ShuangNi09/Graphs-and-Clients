...
class Trip {
    ...
    int reportSegment(int seq, int from, List<Integer> segment) {
        Iterator<Integer> glitter = segment.iterator();
        int jia = glitter.next();
        int yi = glitter.next();
        Road zheyitiao = _map.getLabel(jia, yi);
        Direction zhefang = zheyitiao.direction();
        String zhename = zheyitiao.toString();
        double distzhe = zheyitiao.length();
        Road xiayitiao;
        String xianame;
        String destination = "";
        Direction xiafang;
        ...
        while (glitter.hasNext()) {
            jia = yi;
            yi = glitter.next();
            xiayitiao = _map.getLabel(jia, yi);
            xianame = xiayitiao.toString();
            xiafang = xiayitiao.direction();
            distxia = xiayitiao.length();
            if (zhename.equals(xianame) && zhefang == xiafang) {
                distzhe += distxia;
            } else {
                StringBuilder tobeprint = new StringBuilder();
                tobeprint.append(seq);
                tobeprint.append(". Take ");
                tobeprint.append(zhename);
                tobeprint.append(" ");
                tobeprint.append(zhefang.fullName());
                tobeprint.append(" for ");
                tobeprint.append(round(distzhe, 1));
                tobeprint.append(" miles.");
            ...
            destination = _map.getLabel(yi).toString();
        }
        StringBuilder lastprint = new StringBuilder();
        lastprint.append(seq);
        lastprint.append(". Take ");
        lastprint.append(zhename);
        lastprint.append(" ");
        lastprint.append(zhefang.fullName());
        lastprint.append(" for ");
        lastprint.append(round(distzhe, 1));
        lastprint.append(" miles to ");
        lastprint.append(destination);
        lastprint.append(".");
    ...
    /** Return the distance to 1 decimal place given VALUE
     * and PRECISION. */
    ...
    private void addRoad(String from, String name, double length,
        ...
        Road qu = new Road(name, dir, length);
        Direction fan = dir.reverse();
        Road lai = new Road(name, fan, length);
        _map.add(v0, v1, qu);
        _map.add(v1, v0, lai);
    }
    ...
    private class TripPlan extends SimpleShortestPaths {
        ...
        protected double getWeight(int u, int v) {
            Road lu = _map.getLabel(u, v);
            return lu.length();
        }

        @Override
        protected double estimatedDistance(int v) {
            Location semi = _map.getLabel(v);
            return semi.dist(_finalLocation);
        }
